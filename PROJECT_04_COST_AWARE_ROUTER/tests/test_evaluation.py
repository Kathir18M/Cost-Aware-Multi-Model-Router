...
def test_metric_calculation() -> None:
	records = [
		{"task_type": "qa", "quality": 1, "final_model": "haiku", "initial_model": "haiku", "confidence": 0.9, "actual_cost": 0.01, "baseline_cost": 0.02},
		{"task_type": "qa", "quality": 0.5, "final_model": "sonnet", "initial_model": "haiku", "confidence": 0.6, "actual_cost": 0.03, "baseline_cost": 0.02},
	]
	metrics = calculate_metrics(records)

	assert metrics["total_examples"] == 2
	assert metrics["accuracy"] == 0.5
	assert metrics["haiku_usage_pct"] == 0.5
	assert metrics["sonnet_usage_pct"] == 0.5
...
def test_strategy_comparison_reports_all_required_strategies() -> None:
	...
	def expected(example: dict) -> dict:
		return {**example["expected_output"], "model": "haiku", "confidence": 0.9}

	strategies = {
		"always_haiku": expected,
		"always_sonnet": lambda example: {**expected(example), "model": "sonnet"},
		"cost_aware_router": expected,
	}
	result = evaluate_heldout(strategies)

	assert set(result) == {"always_haiku", "always_sonnet", "cost_aware_router"}
	assert result["always_haiku"]["total_examples"] == 20
	assert result["always_sonnet"]["sonnet_usage_pct"] == 1.0
